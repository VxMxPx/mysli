...
namespace mysli\util\config\script;

__use(__namespace__, '
    ./config AS root/config
    mysli/framework/fs
...
class config {
    ...
    static function run(array $args) {
        ...
        $params->add(
            '--list/-l',
            ['type'    => 'bool',
             'help'    => 'List all packages with configuration values '.
                            'or all configuration options for '.
                            'particular package.']);
        $params->add(
            '--package/-p',
            ['type' => 'str',
     ...
     */
    private static function set_value($package, $key, $value, $string) {
     ...
     * @param  string $package
     */
    private static function get_list($package) {
        if ($package) {
            cout::nl();
