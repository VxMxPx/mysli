...
function __init(array $args) {
    ...
    $params->command = 'tplp';
    $params->description_long = "* If --source and --destination are not ".
        "provided, they'll be set from `mysli.pkg.ym` (tplp section), if not ".
        "defined there, defaults will be used.";
    ...
    $params->add('--destination/-d', [
        'type'    => 'str',
        'default' => '*',
        'help'    => "Directory where templates will be saved. ".
 ...
 * @return null
 */
function observe_or_parse($package, $source, $dest, $loop) {
    ...
    do {
        $rsignature = file::signature(observable_files($source));

        if ($rsignature !== $signature) {
            $changes = what_changed($rsignature, $signature, strlen($source)+1);
...
function observable_files($dir) {
    ...
    foreach ($files as $file) {
        ...
        if (dir::exists($fpath)) {
            $observable = array_merge($observable, observable_files($fpath));
            continue;
        }
