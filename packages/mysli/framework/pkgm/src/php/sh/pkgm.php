...
namespace mysli\framework\pkgm\sh;
...
class pkgm
{
    ...
    static function enable($pkg, $rec=false, $dev=false)
    {
        ...
        $dependencies     = root\pkgm::lst_dependencies($pkg, true);
        // Recommended
        $rec_dependencies = root\pkgm::lst_dependencies($pkg, true, 'recommend');
        // Development
        $dev_dependencies = root\pkgm::lst_dependencies($pkg, true, 'dev');
        ...
        if (count($dependencies['disabled']))
        {
            ...
            {
                if (!self::enable_helper($release, $pkg))
                {
                    return false;
                }
            }
    ...
    static function enable_helper($package, $by)
    {
    ...
    static function disable($pkg)
    {
        ...
        $dependees = root\pkgm::lst_dependees($pkg, true);
        ...
        if (!empty($dependees))
        {
            ...
            foreach ($dependees as $package)
            {
                ...
                if (!self::disable_helper($package))
                {
                    return false;
                }
            }
    ...
    static function disable_helper($package)
    {
        cout::line("Package `{$package}`", false);
    ...
    static function repair()
    {
        ...
        foreach (root\pkgm::lst_enabled(true) as $release => $meta)
        {
            cout::line("    Found `{$release}`", false);
            $dependencies = root\pkgm::lst_dependencies($release, true);
            ...
            if (!empty($dependencies['disabled']))
            {
                ...
                foreach ($dependencies['disabled'] as $ddep)
                {
                    ...
                    {
                        cout::line('        ', false);
                        self::enable_helper($ddep, $release);
                    }
                }
            }
        }
    ...
    static function do_list($option)
    {
        switch ($option)
        {
            case 'enabled':
                cout::line("\n* Enabled packages:");
                cout::line(arr::readable_list(root\pkgm::lst_enabled(), 2));;
                break;

            case 'disabled':
                cout::line("\n* Disabled packages:");
                cout::line(arr::readable_list(root\pkgm::lst_disabled(), 2));
                break;
     ...
     * @return mixed
     */
    static function csi_input(array $properties)
    {
        cout::nl();
     ...
     * @param object $csi
     */
    static function csi_process($csi)
    {
        do {
            switch ($csi->status())
            {
