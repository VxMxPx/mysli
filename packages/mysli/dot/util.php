...
class Util
{
    ...
    /**
     * This will fork execute command, and stop execution in child.
     * Please use 'pcntl_wait($status);' in parent, to prevent zombie processes!
     * --
     * @param  mixed   $commands string|array
     * @param  boolean $print    Print result of script execution (use system rather than exec).
     * --
     * @return integer pid
     */
    public static function fork_command($commands, $print = true)
    {
        $pid = pcntl_fork();
        if ($pid === -1) {
            die(self::error('Cannot fork the process...'));
        } else if ($pid === 0) {
