 ...
 *
 *     array mysli.toolkit, curl.default [
 ...
 *
 *      boolean mysli.toolkit, curl.user_agent, [true]
 *
 * **COSTUME_AGENT**\\
 * A fallaback, if user's agent is not set, or set to false.
 *
 *      string mysli.toolkit, curl.costume_agent, [
 *          Mozilla/5.0 (X11; Linux x86_64; rv:35.0) Gecko/20100101 Firefox/35.0
 *      ]
 *
 * **COOKIE_FILENAME**\\
 * Cookie will be saved in {tmppath}/toolkit.curl/{cookie_filename}
 *
 *      string mysli.toolkit, curl.cookie_filename [cookies.txt]
 */
namespace mysli\toolkit; class curl
{
     ...
     * --
     * @param \mysli\toolkit\curl $curl
     */
    static function set_cookie(\mysli\toolkit\curl $curl)
    {
        $curl->set_opt([
     ...
     * @param array $data
     */
    static function set_post(\mysli\toolkit\curl $curl, array $data)
    {
        $curl->set_opt([
     ...
     * @param array $data
     */
    static function set_get(\mysli\toolkit\curl $curl)
    {
        $curl->set_opt([
            CURLOPT_USERAGENT      => self::get_user_agent(),
            CURLOPT_HEADER         => false,
     ...
     * @param \mysli\toolkit\curl $curl
     */
    static function set_defaults(\mysli\toolkit\curl $curl)
    {
        $curl->set_opt(config::select('mysli.toolkit', 'curl.default', []));
    }
    ...
    {
        if (!$file)
            $file = config::select('mysli.toolkit', 'curl.cookie_filename');

        return fs::tmppath('toolkit.curl', $file);
    }
    ...
    static function get_user_agent()
    {
        ...
        if (isset($_SERVER['HTTP_USER_AGENT']) && $c->get('curl.user_agent'))
