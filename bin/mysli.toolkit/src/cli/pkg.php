...
namespace mysli\toolkit\cli; class pkg
{
    const __use = '
        .{pkg -> root\pkg}
        dot.{prog,param,ui}
    ...
    static function __run(array $args)
    {
        ...
        switch ($prog->get_option_at(0, 'name'))
        {
            ...
            default:
                ui::warn('Invalid command, use --help to see available commands.');
                return false;
        }
    }
    ...
    private static function do_list($type)
    {
        switch ($type) {
            case 'all':
                $list = root\pkg::list_all();
                break;

            case 'enabled':
                $list = root\pkg::list_enabled();
                break;

            case 'disabled':
                $list = root\pkg::list_disabled();
                break;
    ...
    {
        $meta = root\pkg::get_meta($package);
        ui::t(
